...
def load_mnist_data(training_amt=None, test_amt=None):
    training_data = csv_to_data("MNIST_CSV/mnist_train.csv", training_amt)
    print(f"[green]Training data gathered. Length: {len(training_data)}")

    test_data = csv_to_data("MNIST_CSV/mnist_test.csv", test_amt)
...
def train_with_data(model: Neural_Net, training, test, epochs: int):
    ...
    while True:
        save = input("Would you like to save model? (y/n): ")
        if save == "y":
            model.save()
            break
        if save == "n":
