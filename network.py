...
class Neural_Net:
    ...
    def backpropogate(self, inputs, targets):
        ...
        for layer in reversed(self.layers):
            if output_layer:
                ...
                error = error.hadamard(Matrix(1, len(layer.output), [activation_derivative[self.act_func](layer.output)]).transpose())
                prev_weights = layer.weights
                layer.change(error, self.lr)
    ...
    def train_w_batches(self, data, batch_size):
        ...
        for b in range(0, length, batch_size):
            if b in vals:
                ...
                print(f"[red]Progress Update: {b/length*100}, Time: {end - start}")
                start = end
    ...
    def train_w_shuffle(self, data, steps):
        # will randomize data
        # data = [{"input": __, "output": __}, ...]
        for _ in range(steps):
            value = data[random.randint(0, len(data)-1)]
            self.backpropogate(value["input"], value["output"])
    ...
    def train(self, data):
        ...
        for i, d in enumerate(data):
            if i in vals:
                ...
                print(f"[red]Progress Update: {i/length*100}, Time: {end - start}")
                start = end
...
class Layer:
    ...
    def change(self, error, lr):
        ...
        self.biases = add_vects(self.biases, matrix_converter(delta_B))
